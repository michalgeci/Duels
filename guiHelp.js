...
export class GuiHelp {
  ...
  addToScene = (addedElement, x, y) => {
    for (let rowIdx = 0; rowIdx < addedElement.length; rowIdx++) {
      this.rows[rowIdx + y].innerHTML = this.replaceAt(
        this.rows[rowIdx + y].innerHTML,
        x,
        addedElement[rowIdx]
      );
    }
  ...
  replaceAt = (text, index, replacement) => {
    ...
    return (text.substr(0, idx) + repl + text.substr(idx + repl.length)).substr(
      0,
      text.length - 0
    );
  };
  ...
  showOpponentReactionTime = (opponentTime) => {
    this.addToScene(
      ["Opponent reaction time: " + opponentTime + " ms"],
      21,
      28
    );
  };
   ...
   */
  setScore = (hostScore, clientScore) => {
    this.addToScene([hostScore.toString().padStart(3, '0')], 34, 2);
    this.addToScene([clientScore.toString().padStart(3, '0')], 40, 2);
  };
}
